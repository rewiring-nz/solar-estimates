...
CREATE OR REPLACE TABLE building_calc_attribs(
    fid INTEGER PRIMARY KEY,
    building_id INTEGER,    
...
CREATE OR REPLACE TABLE roof_segment(
    fid INTEGER PRIMARY KEY,
    geom POLYGON_2D,
    building_id INTEGER,
    FOREIGN KEY (building_id) REFERENCES building(fid),
...
-- v2
CREATE OR REPLACE TABLE roof_segment_calc_attribs (
    fid INTEGER PRIMARY KEY,
...
-- Missing primary key?
CREATE OR REPLACE TABLE grid_cell_elevation (
    -- (lat,long) coordinate
    coord POINT_2D,
...
-- v1
-- Missing foreign key?
CREATE OR REPLACE TABLE grid_cell_calc_attribs (
    run_id INTEGER,
    FOREIGN KEY (run_id) REFERENCES run(fid),
...
-- v1
CREATE OR REPLACE TABLE weather_region (
    fid INTEGER PRIMARY KEY,
...
-- v3
CREATE OR REPLACE TABLE hourly_building_energy (
    fid INTEGER PRIMARY KEY,
    building_id INTEGER,    
    FOREIGN KEY (building_id) REFERENCES building(fid),
    run_id INTEGER,
